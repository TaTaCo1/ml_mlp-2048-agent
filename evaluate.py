...
from mcts import DQNAgentWithMCTS
import torch
import json
...
def compare_all(episodes, model_path, model_mcts_path, top_n=1):
    ...
    print("="*70)
    print(f"{'Agent':<20} {'Avg Score':<15} {'Avg Max Tile':<15}")
    print("-" * 50)
    print(f"{'Random':<20} {rand_score:<15.0f} {rand_tile:<15.0f}")
    print(f"{'DQN only':<20} {dqn_score:<15.0f} {dqn_tile:<15.0f}")
    print(f"{'DQN + MCTS':<20} {mcts_score:<15.0f} {mcts_tile:<15.0f}")
    print("="*70 + "\n")
    print("Files saved:")
    print("  top_games_dqn.json")
    print("  top_games_mcts.json")
    print("  top_games_random.json")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate 2048 agents")
    ...
    if args.mode == "all":
        compare_all(args.episodes, args.model, args.model_mcts,
                    args.top_n)
    elif args.mode == "dqn":
        evaluate_dqn(args.episodes, args.model, args.top_n)
    elif args.mode == "mcts":
        evaluate_dqn_mcts(args.episodes, args.model_mcts,  # ← uses mcts model
                        args.top_n)
